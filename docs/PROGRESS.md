...
- **What landed**: `_mam_pcarbon_aging_1subarea` in `mam4_jax/coupling/amicphys.py` — the port of `modal_aero_amicphys.F90:5111-5285`, the sulfate-monolayer coating criterion that converts aged primary-carbon particles to the accumulation mode. Called after coag (Fortran `:2555`) so the shell includes both this step's condensed and coagulated so4/soa, and gated by a new `mdo_pcarbonaging` toggle (default 1 — the Fortran has no namelist switch; the `*_no_pcarbon_aging` fixtures were captured with `skip_pcarbon_aging.patch`). This was the last amicphys sub-process out of scope in `FEATURES.md`.
- **Closes the pcarbon repack leak**: the pcarbon `LMAP_AER` row maps only pom/bc/mom, so so4/soa condensed onto that mode had no pcnst slot and was silently dropped at the amicphys repack — a standing per-step sulfur/SOA sink. Aging transfers the unmapped shell species wholesale to accum before the repack, which is exactly what rescues that mass. `test_aging_closes_the_pcarbon_sulfur_leak` closes gas+aerosol sulfur to the netprod source at 1e-9 with aging on, and reproduces the leak at >100× that residual with it off.
- **Threshold default is 3.0** (`configure_pcarbon_aging(n_so4_monolayers=…)` for the process-global default; per call via the `AmicphysParams` pytree — see the `AmicphysParams` bullet below). The amicphys path receives the value via `phys_control` (`box_model_utils/phys_control.F90:26` → `modal_aero_initialize_data.F90:417,585`); the frequently-quoted 8.0 (`modal_aero_gasaerexch.F90:37`) belongs to the legacy `modal_aero_coag` aging path. Defaults reproduce the reference: at 8.0 the 60-step trajectory is 434 % off on pcarbon tracers, at 3.0 it is 2.07 %.
- **Float32 coagulation mass transfer was identically zero.** `getcoags`' harmonic means used the product form `a*b/(a+b)`; the products underflow float32 min-normal while the operands do not, so `betaij3` — and therefore *all* intermodal third-moment (mass) coag transfer — flushed to 0 in an f32 core. Number transfer was unaffected, so nothing looked broken. All seven harmonic means now route through a reciprocal-form `_harmonic_mean_safe`; `test_wrapper_all_outputs_nonzero_in_float32` locks all 8 wrapper outputs. This resolves plan 023 §2's deferred "audit other coag coefficients" row, whose premise ("only qv12 is f32-broken") was wrong — four second-moment coefficients were dead too, and so was `betaij3`, which PR #60 believed it had fixed (factoring `dgat3` out kept the operands in range but left the product form intact). Correction appended as plan 023 §8: the old `test_getcoags_finite_in_float32` second-moment tier asserted finite-only, and zero is finite, so the assertion and the defect were mutually invisible.
- **ADR-019 (new)**: records the two deliberate departures from the Fortran's arithmetic — `1-exp(-x)` → `-expm1(-x)`, and the `qs21` prefactor rewrite. The second leaves `qs21` at 1.19e-7 vs the Fortran (every other coefficient stays at machine ε) because the Fortran expression cancels catastrophically at large diameter ratios (wrong sign at `dgacc/dgatk = 1e6`, in float64). Both items accepted on review; the rewrite is kept on both precisions rather than scoped to f32, and `tests/test_coag.py`'s `RTOL` carries a note explaining why `qs21` has ~7 orders less margin than its siblings.
- **Validation**: `tests/test_pcarbon_aging.py` (9 tests) — hand-computed criterion at machine ε, mass/number conservation, saturation and zero-core edges, sulfur closure, and end-to-end `run_step` / 60-step parity against the canonical aging-ON `per_process/` bundle (previously unused) at the ADR-015 coarse-dt bars: aerosol ≤ 2.1 %, gas ≤ 5.3 %. The two Fortran builds differ by up to 14× on pcarbon tracers, so this is a real constraint rather than a tautology.
- **`AmicphysParams` — numeric knobs become differentiable traced leaves (ADR-020)**: `amicphys`'s long-reserved `params` slot now takes an `AmicphysParams` NamedTuple (a JAX pytree) carrying `n_so4_monolayers`, `qgas_netprod_h2so4` and `qgas_netprod_soa`. Fields left `None` resolve to the `configure_*` globals at trace time, so those hooks survive as process defaults. `run_step`/`run_timesteps` thread it as a *traced* argument — `static_argnames` is back to the structural toggles only — so a threshold sweep reuses one compiled step and `jax.grad` flows to the threshold, the calibration target M9 needs. **There is no per-call `n_so4_monolayers` keyword**; use `params`. Code-path selectors (condensation `backend`, the `mdo_*` toggles) deliberately stay out of the struct, and `mdo_pcarbonaging` is *not* expressible through the threshold — `n_so4_monolayers=0` means instant full aging, not off. The threshold is clamped at 0 (`jnp.maximum`): negative values could never reverse the transfer (the `max(0, tmp2)` guard mirroring F90:5234 already collapsed them to the saturated branch), but as a trainable leaf a negative excursion would park an optimizer in a flat zero-gradient region silently.
- **Plan**: `docs/plans/026-pcarbon-aging.md`.
- **Test suite**: 180 passed (was 170).
