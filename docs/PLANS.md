...
3. [ ] **`modal_aero_wateruptake_dr`** (`modal_aero_wateruptake.F90:130-150`) — equilibrium water uptake. First aerosol-state-aware port. Reference data: `tests/reference/per_process/wateruptake_{before,after}.npz` with the now-populated index tables to map them onto JAX state.
