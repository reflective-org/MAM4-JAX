...
4. **`modal_aero_calcsize_sub`** (`modal_aero_calcsize.F90`) — size redistribution. Two-PR bottom-up split:
   ...
   - 4b. [ ] **PR-B**: Aitken ↔ accumulation mode-transfer block (Fortran lines 944–1380). Validated against the existing `tests/reference/per_process/calcsize_{before,after}.npz` (full-transfer enabled). Branch is a no-op in the canonical reference (see `docs/DEFERRED.md`).
