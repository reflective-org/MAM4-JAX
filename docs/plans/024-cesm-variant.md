...
| **B** | **DONE** — branch `feat/topology-parameter`, 139 tests green. **Topology as a parameter.** Replace `data.py`'s module-level constants with a frozen `Topology` dataclass plus named instances (`E3SM_MAM4_MOM`, `CAM_MAM4`, `CAM_MAM5`), keeping the module names as aliases to the default. Pure refactor; E3SM path **bit-unchanged**. Also folds in revision 1's PR B: scattered Layer-3 values into `config.py`, `numerics.py` for Layer 4 | A |
| **C** | ⚠ **First: thread the topology as a jit STATIC ARGUMENT, never a module global read inside a kernel body.** jit caches are not keyed on module globals, so a kernel traced under one topology silently keeps that answer after `set_topology` — verified empirically in the PR-B review. `Topology` is frozen and hashable precisely so it can be a `static_argnums` argument. Requires a test asserting a topology switch *changes* a jitted result. Then: CAM species properties: hygroscopicity, the four `specmw_amode` sets, `MW_SO4A_HOST`. Sourced from `mam-box-fortran/params/cam_mam4_params.py` (generated from the physprop NetCDF with SHA256 provenance) | B |
