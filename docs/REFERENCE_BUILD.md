...
| `instrumented-gasaerexch-only` | Same hooks but with `mdo_gasaerexch=1, others=0` plus two extra patches: `gasaerexch_skip_soaexch.patch` (skips the `mam_soaexch_1subarea` call inside gasaerexch) and `skip_pcarbon_aging.patch` (skips `mam_pcarbon_aging_1subarea` inside the sub-area handler). Both patches are necessary so the Fortran fixture matches M3.6 PR-D's port scope. Used by `test_orchestration_gasaerexch_only_matches_fortran`. | overlay + skip_soaexch + skip_pcarbon_aging | `tests/reference/per_process_gasaerexch_only/*.npz` |
