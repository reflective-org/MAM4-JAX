...
- **Status:** Accepted 2026-08-25. Introduced alongside PR [#75](https://github.com/reflective-org/MAM4-JAX/pull/75) (plan 026) — pcarbon aging + float32-safe coag. Both items accepted by the owner on review; item 2 was held for explicit sign-off because it is the first place the port knowingly disagrees with the reference in float64.
...
- **Decision:**
  ...
  2. **`qs21`'s `(1+r6)^(2/3) - rx4` → `rx4 · expm1((2/3) · log1p(1/r6))`** (`getcoags`, Fortran `modal_aero_coag.F90:2716`). Exact identity: `rx4 = r⁴` and `r6 = r⁶`, so `r6^(2/3) = rx4` and `(1+r6)^(2/3) - rx4 = rx4·[(1+1/r6)^(2/3) - 1]`. The Fortran form is a difference of two nearly-equal terms whose cancellation cost grows with the diameter ratio; at `dgacc/dgatk = 1e6` it returns `−1.587e-3` where the true value is `+6.67e-7` — **wrong sign, in float64**. The rewrite is stable in both precisions. **Accepted**: the rewrite is more correct rather than a trade — it is kept on BOTH precisions, not scoped to float32, because a dtype-branched duplicate of one expression would preserve bit-parity with a formula we have established is defective. The cost is `qs21`'s reduced margin against the reference test; see Consequences, and the note at `tests/test_coag.py`'s `RTOL`.
...
- **Alternatives considered:**
  - **Scope the `qs21` rewrite to the f32 path only**, leaving f64 bit-faithful to the Fortran. Would keep the reference test at machine ε across any sweep width. **Rejected on owner review**: it carries two spellings of one expression forever, and leaves the f64 path deliberately computing a value that is wrong (wrong *sign* at large diameter ratios) purely to match the reference bit-for-bit. Since `qs21 → betaij2j` is discarded by `_mam_coag_1subarea`, no result observable today differs between the two options — so the tiebreaker is which expression a future reader should inherit, and that is the correct one.
...
## ADR-020 — Numeric knobs are traced pytree leaves; code-path selectors stay static

- **Status:** Accepted 2026-08-25. Introduced alongside PR [#75](https://github.com/reflective-org/MAM4-JAX/pull/75) (plan 026) — `AmicphysParams`.
- **Context:** Every tunable amicphys number had accumulated into process-global `configure_*` dicts read at JIT trace time (ADR-017's `configure_condensation`, PR #61's `configure_gas_netprod`, plan 026's `configure_pcarbon_aging`). That mechanism has three costs, and pcarbon aging's threshold made all three bite at once: it is **not differentiable** (a trace-time Python float never appears in the graph), it forces **one recompile per value** when it was additionally exposed as a static per-call argument, and it carries a **documented stale-cache footgun** (a `None`-defaulted call bakes the global into a cached trace that later `configure_*` calls do not invalidate). `n_so4_monolayers` is a genuinely uncertain parameter — reference models span 1.0 to 8.0, and it sets BC/POM lifetime directly — so it is exactly what M9 calibration wants a gradient through. Nothing in the physics requires it to be static: it enters the criterion as smooth arithmetic.
- **Decision:**
  1. **Numbers the physics is differentiable with respect to become traced pytree leaves.** `AmicphysParams` (a `NamedTuple`, so a pytree for free) carries `n_so4_monolayers`, `qgas_netprod_h2so4`, `qgas_netprod_soa`, passed through `amicphys`'s long-reserved `params` slot and threaded by `run_step` / `run_timesteps` as an ordinary traced argument.
  2. **Code-path selectors stay static.** The condensation `backend` and the `mdo_*` toggles choose *which* code is traced and remain static configuration/arguments. The split mirrors the `flax.struct` convention: pytree leaves for numbers you might differentiate, static aux for branch selectors.
  3. **`mdo_pcarbonaging` is not folded into the threshold.** `n_so4_monolayers = 0` is a zero-thickness coating requirement — the whole mode ages *instantly* — and no finite value suppresses the wholesale shell-species transfer. Only the toggle reproduces the `skip_pcarbon_aging` reference build. Off and full-on are not opposite ends of one dial.
  4. **`configure_*` survives as the process-global default.** A `None` field resolves to its configured value at trace time, so existing hosts are unaffected and the hooks remain the "set it once at startup" path.
  5. **Trainable leaves get their physical floor made local.** `n_so4_monolayers` is clamped with `jnp.maximum(n, 0)`. A negative value could never reverse the transfer even unclamped — the `max(0, tmp2)` guard mirroring `modal_aero_amicphys.F90:5234` already collapses it to the saturated branch — but an optimizer that wanders negative would sit in that flat, exactly-zero-gradient region without any signal. Calibration search bounds should stay positive.
- **Consequences:**
  - `jax.grad` flows to the aging threshold and the H₂SO₄ production rate through the full `amicphys` call; a threshold sweep reuses one compiled step instead of recompiling per value.
  - **There is no per-call `n_so4_monolayers` keyword.** The static per-call argument that briefly existed on this branch is replaced by `params`; `run_step(state, AmicphysParams(n_so4_monolayers=3.0))`.
  - The stale-cache footgun is *bypassable* but not *gone*: it still applies to any call that leaves a field `None`. `configure_pcarbon_aging`'s docstring documents both.
  - This is the pattern for future numeric knobs. New tunables should land as `AmicphysParams` fields, not as new `configure_*` globals; new *selectors* should stay static.
  - `qgas_netprod_soa` remains stored-not-consumed, exactly as its `configure_gas_netprod` counterpart (see `DEFERRED.md`, "SOA netprod source"). Being a pytree leaf does not make it live.
- **Alternatives considered:**
