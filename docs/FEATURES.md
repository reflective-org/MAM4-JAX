...
| Gas–aerosol exchange (condensation) | `e3sm_src_modified/modal_aero_amicphys.F90` (`mam_gasaerexch_1subarea`, lines 3279–3584) | **ported (validated) and wired into the orchestration** as `_mam_gasaerexch_1subarea` in `mam4_jax/processes/amicphys.py` (M3.6 PR-D). H₂SO₄ analytical-solver path + 3 leaf helpers (`mean_molecular_speed`, `gas_diffusivity`, `gas_aer_uptkrates_1box1gas`). Max rel-err **7.8e-16** (machine ε) on the 5 gasaerexch-modified tracers across 60 timesteps. SOA exchange (`mam_soaexch_1subarea`) and the RK4 branch are out of scope (PR-E for SOA). Stub `mam4_jax/processes/gasaerexch.py` remains dead code. |
...
| Capability | Status |
| --- | --- |
| Element-wise `1e-6` rel-err assertion (ADR-003) | **in use** — every M3 port PR has an end-to-end test asserting max rel-err < 1e-6 against a committed Fortran capture. As of M3.6 PR-D, ten ports plus the orchestration unpack/repack round-trip all pass. Most ports match at machine ε; rename matches at ~1e-9; orchestration's gasaerexch-wired-through test matches at < 1e-15 against the gasaerexch-only single-toggle Fortran capture. |
...
| Residual / convergence diagnostic plots | **in use** — nine plots committed under `docs/figures/` (polysvp, qsat, makoh, kohler, wateruptake, calcsize, rename, gasaerexch residuals + the upstream flowchart). New ports add their plot per the validation workflow in `CLAUDE.md`. |
