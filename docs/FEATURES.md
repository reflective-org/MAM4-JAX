...
| Capability | Status |
| --- | --- |
| Element-wise `1e-6` rel-err assertion (ADR-003) | **in use** — every M3 port PR has an end-to-end test asserting max rel-err < 1e-6 against a committed Fortran capture. As of M3.6 PR-C, nine ports plus the orchestration unpack/repack round-trip all pass. First eight ports match at machine ε; rename matches at ~1e-9 (first port with non-trivial physics actually exercised by the canonical fixture); orchestration's rename-wired-through test matches at < 1e-12 against the rename-only single-toggle Fortran capture. |
