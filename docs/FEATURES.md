...
| Coagulation (Brownian, intra/inter-modal) | `e3sm_src_modified/modal_aero_amicphys.F90` (`mam_coag_1subarea`, lines 4670–5106) + `e3sm_src/modal_aero_coag.F90` (`getcoags`, `getcoags_wrapper_f`) | **leaf ported (validated)** — M3.6 PR-G1: `getcoags` (closed-form Whitby coagulation coefficients, ~1685 LOC including ~1200 LOC of lookup tables) in `mam4_jax/coag.py`. Max rel-err **6.5e-9** on all 8 coefficients across 240 records. PR-G2 (wrapper) and PR-G3 (subarea orchestration + amicphys wiring) outstanding. Stub `mam4_jax/processes/coag.py` and the no-op `_mam_coag_1subarea` stub inside `amicphys.py` remain pending PR-G3. |
