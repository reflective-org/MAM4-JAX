...
| Gas–aerosol exchange (condensation) | `e3sm_src_modified/modal_aero_amicphys.F90` (`mam_gasaerexch_1subarea`, lines 3279–3584, with `mam_soaexch_1subarea` 3589–3918) | **ported (validated) and wired into the orchestration** as `_mam_gasaerexch_1subarea` + `_mam_soaexch_1subarea` in `mam4_jax/processes/amicphys.py` (M3.6 PR-D for H₂SO₄ + helpers, PR-E for SOA exchange). SOA path uses the single-substep assumption (`dtcur = dtfull`); adaptive sub-stepping deferred to PR-E2 if a fixture ever triggers it. Max rel-err **4.77e-15** (machine ε) on all SOA/H₂SO₄/so4 tracers across 60 timesteps. RK4 branch still out of scope. Stub `mam4_jax/processes/gasaerexch.py` remains dead code. **Three condensation backends now selectable via `amicphys.configure_condensation` (PR #59, plan 022)** — `"diffrax"` (default, audited grad-clean per PR-J5), `"substep"` (operator-split closed-form + frozen-`g_star` fixed-N substep, grad-OK via `lax.scan`, ~55× faster), `"astem"` (Fortran-faithful adaptive semi-implicit Euler via `lax.while_loop`, ~38× faster, **NOT reverse-mode differentiable**). See ADR-017 for the per-call equivalence bar (`rtol=1e-2`). The H₂SO₄ "other-process" production stub (`driver.F90:1248`, `1e-16 mol/mol/s`) is now the default of `amicphys.configure_gas_netprod` (PR #61) — hosts with their own sulfur chemistry pass `h2so4=0.0`; the `soa` rate is stored but not yet consumed (see `DEFERRED.md` "SOA netprod source"). |
...
| Primary-carbon aging (pcarbon → accum monolayer transfer) | `e3sm_src_modified/modal_aero_amicphys.F90` (`mam_pcarbon_aging_1subarea`, lines 5111–5285) | **ported (validated) and wired into the orchestration** as `_mam_pcarbon_aging_1subarea` in `mam4_jax/coupling/amicphys.py`, called after coag (Fortran line 2555), gated by `mdo_pcarbonaging` (default **1** — the Fortran has no toggle; the `*_no_pcarbon_aging` fixtures were captured with `skip_pcarbon_aging.patch`). Monolayer threshold set process-globally via `configure_pcarbon_aging(n_so4_monolayers=...)`, or per call — and differentiably — as an `AmicphysParams` leaf (`run_step(state, AmicphysParams(n_so4_monolayers=...))`, ADR-020); default 3.0, the amicphys-path reference value (phys_control → `modal_aero_amicphys_init`, `box_model_utils/phys_control.F90:26`); the oft-quoted 8.0 (`modal_aero_gasaerexch.F90:37`) belongs to the legacy `modal_aero_coag` aging path; ECHAM-HAM's counterpart criterion (`m7_coat`) uses 1.0. The cond/coag budget-attribution split (F90:5169-5210 + `qaer_del_coag_in`) is NOT ported — those arrays are diagnostics this port doesn't carry, and `vol_shell` uses only the current composition, so state evolution is unaffected. Also closes the pcarbon repack leak (condensed so4/soa on pcarbon has no pcnst slot and was silently dropped). Validated: hand-computed criterion at machine ε, sulfur closure, plus `run_step`/60-step trajectory vs the CANONICAL (aging-on) `per_process/` bundle at the ADR-015 bars (`tests/test_pcarbon_aging.py`). |
...
| Whitby coagulation coefficients (closed-form intra/intermodal, 0th/2nd/3rd moments) | `modal_aero_coag.F90:1177-2858` (`getcoags`) + lookup tables in `modal_aero_coag.F90:1306-2540` | **ported (validated)** as `getcoags` in `mam4_jax/coag.py` (M3.6 PR-G1). Lookup tables (`bm0`, `bm0ij`, `bm3i`, `bm2ii`, `bm2iitt`, `bm2ij`, `bm2ji`) extracted once by `scripts/extract_coag_tables.py` into `mam4_jax/_coag_tables.npz`. Max rel-err **6.5e-9** on all 8 coefficients across 240 (T, P, dgnumA, dgnumB) records. **Float32-safe (PR #60 / plan 023, ADR-018):** the `qv12` third-moment coefficient is recomputed via a `dgat3`-factored harmonic mean so its two `~1e-38` operands stay in float32 normal range — qv12 is finite at rel-err ~5.9e-8 vs the f64 reference. The `qs*` second-moment coefficients (`qs11`, `qs22`, `qs12`, `qs21`) remain magnitudes 1e-30 to 1e-27, below float32's useful precision floor; they stay finite but are not numerically meaningful in float32. |
