...
| Rename (Aitken → accumulation) | `e3sm_src_modified/modal_aero_amicphys.F90` (`mam_rename_1subarea`, lines 3923–4246) | **ported (validated)** as `_mam_rename_1subarea` inside `mam4_jax/processes/amicphys.py` (M3.6 PR-B). Max rel-err 2.5e-9 (qnum) / 7.0e-10 (qaer) across 60 timesteps of the canonical full-physics fixture. Operates on the amicphys-local view; orchestration-shell wiring deferred to PR-C alongside `_mam_gasaerexch_1subarea`. Stub `mam4_jax/processes/rename.py` remains dead code in the box-model build. |
...
| Capability | Status |
| --- | --- |
| Element-wise `1e-6` rel-err assertion (ADR-003) | **in use** — every M3 port PR has an end-to-end test asserting max rel-err < 1e-6 against a committed Fortran capture. As of M3.6 PR-B, nine ports (polysvp, qsat_water/ice, IndexTables, makoh_cubic/quartic, modal_aero_kohler, wateruptake_dr, calcsize_sub, mam_rename_1subarea) all pass; first eight match at machine ε, rename matches at ~1e-9 (the first port with non-trivial physics actually exercised by the canonical fixture). |
...
| Residual / convergence diagnostic plots | **in use** — eight plots committed under `docs/figures/` (polysvp, qsat, makoh, kohler, wateruptake, calcsize, rename residuals + the upstream flowchart). New ports add their plot per the validation workflow in `CLAUDE.md`. |
