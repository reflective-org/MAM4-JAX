...
| Operator-splitting time loop (driver) | `test_drivers/driver.F90:1080-1367` (`main_time_loop`) | **ported and validated end-to-end (60 steps)** in `mam4_jax/driver.py` (M4 PR-A scaffold + PR-B trajectory validation). `run_step(state)` chains `calcsize → wateruptake → cloud_chem (no-op) → amicphys`; `run_timesteps(state, n_steps)` is a plain Python `for` loop returning a stacked-snapshot trajectory (`jax.lax.scan` deferred to M6). **Max rel-err 1.97e-8** on `q` across 60 steps × 35 tracers — 50× under ADR-003. Cloud-chem stub fires the no-op branch (box-model fixture has `cldn=0`); pcarbon-aging remains out of scope; gas-chem term stays inside gasaerexch's analytical solver. Figure: `docs/figures/driver_60step_trajectory.png`. |
