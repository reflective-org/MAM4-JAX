...
| Operator-splitting time loop (driver) | `test_drivers/driver.F90:1080-1367` (`main_time_loop`) | **scaffolded** in `mam4_jax/driver.py` (M4 PR-A): `run_step(state)` chains `calcsize → wateruptake → cloud_chem (no-op) → amicphys`; `run_timesteps(state, n_steps)` is a plain Python `for` loop returning a stacked-snapshot trajectory (`jax.lax.scan` deferred to M6). 1-step test against `per_process_full_minus_pcarbon_aging/` reaches max rel-err **2.5e-9** on `q`. M4 PR-B will validate the 60-step trajectory and produce the mode-by-mode size-distribution comparison figure. Cloud-chem stub fires the no-op branch (box-model fixture has `cldn=0`); pcarbon-aging remains out of scope. |
