...
| Coagulation (Brownian, intra/inter-modal) | `e3sm_src_modified/modal_aero_amicphys.F90` (`mam_coag_1subarea`, lines 4670–5106) + `e3sm_src/modal_aero_coag.F90` (`getcoags`, `getcoags_wrapper_f`) | **leaves ported (validated)** — M3.6 PR-G1 + PR-G2: `getcoags` (closed-form Whitby coagulation coefficients) + `getcoags_wrapper_f` (prep + CMAQ→MIRAGE2 post-processing) in `mam4_jax/coag.py`. Max rel-err **6.5e-9** on all 8 raw outputs and 8 post-processed beta coefficients across 240 records. PR-G3 (subarea orchestration + amicphys wiring) outstanding. Stub `mam4_jax/processes/coag.py` and the no-op `_mam_coag_1subarea` stub inside `amicphys.py` remain pending PR-G3. |
