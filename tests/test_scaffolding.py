...
#
# These four names once had `mam4_jax/processes/*.py` modules that raised
# NotImplementedError. They were dead code -- nothing imported them -- and worse,
# two of them SHADOWED working implementations by name: `mam4_jax.coag` and
# `mam4_jax.newnuc` were real, so `from mam4_jax.processes import coag` (the
# obvious guess) got you a crash while `from mam4_jax import coag` worked.
# They were deleted in the package restructure. The physics for gasaerexch and
# rename lives in `mam4_jax/coupling/amicphys.py` as `_mam_*_1subarea` helpers.
DELETED_STUB_NAMES: tuple[str, ...] = (
    "gasaerexch",
    "newnuc",
    "coag",
    "rename",
...
@pytest.mark.parametrize("module_name", DELETED_STUB_NAMES)
def test_dead_process_stubs_stay_deleted(module_name: str) -> None:
    """The old NotImplementedError stubs must not come back.

    Re-adding one would shadow a working implementation by name again, which is
    the failure this guards: it looks like the right import and raises at
    runtime. Asserting absence is the successor to the old test that asserted
    they raise.
    """
    with pytest.raises(ModuleNotFoundError):
        importlib.import_module(f"mam4_jax.processes.{module_name}")


def test_the_processes_subpackage_is_gone() -> None:
    """`processes/` split the science arbitrarily -- calcsize and wateruptake
    were in it while coag, newnuc, kohler and saturation were not. It was
    replaced by `physics/` (science), `coupling/` (host-model sequencing) and
    `core/` (tables and configuration)."""
    with pytest.raises(ModuleNotFoundError):
        importlib.import_module("mam4_jax.processes")
